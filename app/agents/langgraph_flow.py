...
def _make_stage_node(stage_id: int):
    def _node(state: AssessState) -> dict:
        ...
        turn = AE.run_turn(owner, stage_id, dossier, learnings, msg, A.METHODOLOGY)
        ...
        while getattr(turn, "handoff", None) and hops < _MAX_HANDOFFS:
            turn = AE.run_turn(turn.handoff, stage_id, dossier, learnings, msg, A.METHODOLOGY)
            transcript.append({"stage": stage_id, "agent": turn.agent_id, "body": turn.body})
            hops += 1
...
def build_graph(session=None):
    ...
    for st, name in zip(A.STAGES, names):
        g.add_node(name, _make_stage_node(st.id))
